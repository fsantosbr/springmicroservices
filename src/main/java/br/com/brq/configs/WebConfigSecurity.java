...
public class WebConfigSecurity extends WebSecurityConfigurerAdapter {
	...
	private static final String[] PUBLIC_ENDPOINT = { "/v2/api-docs", "/configuration/ui", "/swagger-resources/**",
			"/configuration/security", "/swagger-ui.html", "/webjars/**" };

	private static final String[] PUBLIC_ENDPOINT_GET = { "/produtos", "/csrf", "/" };
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.cors().and().csrf().disable().authorizeRequests().antMatchers(HttpMethod.POST, PUBLIC_ENDPOINT_POST)
				.permitAll().antMatchers(HttpMethod.GET, PUBLIC_ENDPOINT_GET).permitAll().antMatchers(PUBLIC_ENDPOINT)
